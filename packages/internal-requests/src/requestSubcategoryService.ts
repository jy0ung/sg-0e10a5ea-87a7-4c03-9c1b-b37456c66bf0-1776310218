...
import { buildRequestSubcategoryKey } from './requestSubcategories';
...
export async function updateRequestSubcategory(
  ...
  context: RequestSubcategoryContext,
): Promise<ConfigMutationResult<RequestSubcategoryRecord>> {
  ...
  if (!data) {
    return expectedUpdatedAt
      ...
      : { data: null, error: 'Subcategory not found.' };
  }

  void logUserAction(context.actorId, 'update', 'request_subcategory', subcategoryId, {
    component: 'RequestSubcategoryService',
    ...buildAuditDiff(currentSubcategory as unknown as Record<string, unknown>, patch),
  });
