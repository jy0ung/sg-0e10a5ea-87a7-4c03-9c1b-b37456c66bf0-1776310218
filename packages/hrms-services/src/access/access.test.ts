...
import type { HrmsRole } from '@flc/types';
...
describe('deriveHrmsAccess', () => {
  it('grants self-service routes to staff without exposing admin modules', () => {
    ...
    expect(access.canAccessRoute('myAttendance')).toBe(true);
    expect(access.canAccessRoute('attendancePage')).toBe(true);
    expect(access.canAccessRoute('teamLeave')).toBe(false);
  });
  ...
  it('grants management routes from HRMS roles instead of any main-app role', () => {
    ...
    expect(access.canAccessRoute('teamLeave')).toBe(true);
    expect(access.canAccessRoute('leaveQuota')).toBe(true);
