...
export interface AuthErrorTracker {
...
export function AuthProvider({
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    const sendToPending = (reason: string) => {
      logger?.warn(reason, { userId }, 'AuthContext');
      setProfile(null);
    ...
    try {
      ...
      if (error) {
        logger?.error('Error fetching profile', { error }, 'AuthContext');
      ...
      setProfileError(null);
      setProfile(p);
      logger?.setUserId(p.id);
      errorTracker?.setUser(p.id);
    } catch (err) {
      logger?.error('Unexpected error fetching profile', { error: err }, 'AuthContext');
      setProfileError('Unexpected error loading your profile. Please sign in again.');
      await supabase.auth.signOut();
      clearSessionArtifacts();
    }
  }, [clearSessionArtifacts, logger, errorTracker]);
  ...
  const login = useCallback(async (email: string, password: string) => {
    try {
      ...
      logger?.error('Unexpected sign-in error', { error: err }, 'AuthContext');
