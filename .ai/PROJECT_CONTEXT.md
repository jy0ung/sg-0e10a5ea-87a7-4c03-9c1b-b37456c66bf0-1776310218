...
- 2026-05-08 live note: Supabase Auth was manually restarted with the existing Resend credential in-process and now reports `GOTRUE_RATE_LIMIT_EMAIL_SENT=30`. `/etc/flc-bi/supabase.env` still did not exist after this change because the current shell could not perform non-interactive sudo. For reboot persistence, a sudo-capable session must write `AUTH_SMTP_PASS` into `/etc/flc-bi/supabase.env` via `scripts/configure-supabase-auth-smtp.sh` or equivalent.
...
- 2026-05-08: Applied live rate-limit change by restarting Supabase with the existing Resend credential from the running Auth container. Verified live GoTrue reports `GOTRUE_RATE_LIMIT_EMAIL_SENT=30` and `/auth/v1/health` returns 200. Root-owned systemd env persistence remains pending.
