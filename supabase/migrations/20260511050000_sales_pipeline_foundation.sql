-- ============================================================================
-- Migration: 20260511050000_sales_pipeline_foundation.sql
-- Purpose:   Stage 3 — Sales Pipeline Foundation
--            • transition_sales_order_stage() — audited pipeline move RPC
--            • get_sales_pipeline_summary()    — per-stage counts + values
--            • get_sales_dashboard_summary()   — server-side Sales Dashboard KPIs
-- Author:    Generated 2026-05-11
-- ============================================================================

-- ============================================================================
-- 1.  transition_sales_order_stage
-- ============================================================================
-- Atomically moves a sales_order to a new deal_stage and writes an audit_logs
-- row.  Enforces company scope: the order AND the target stage must belong to
-- p_company_id.  A NULL p_stage_id un-assigns the order from the pipeline.
-- Returns JSON: { action, order_id, previous_stage_id, new_stage_id }
-- ============================================================================
...
BEGIN
  -- Company-scope check + capture previous stage
...
-- ============================================================================
-- 2.  get_sales_pipeline_summary
-- ============================================================================
-- Returns pipeline counts and values grouped by deal_stage for kanban views.
-- Includes both stage buckets and an "unassigned" bucket for orders with no
-- deal_stage_id.  Optional filters: branch_code, date range.
--
-- Return shape (jsonb):
--   {
--     by_stage: [ { deal_stage_id, stage_name, stage_order, stage_color,
--                   order_count, delivered_count, total_value } ],
--     unassigned: { order_count, delivered_count, total_value },
--     totals:     { order_count, delivered_count, total_value }
--   }
-- ============================================================================
    ...
    'by_stage', COALESCE((
      ...
      FROM (
        SELECT
          ...
          COUNT(so.id) FILTER (WHERE so.status = 'delivered')            AS delivered_count,
          COALESCE(SUM(so.total_price), 0)                               AS total_value
        FROM deal_stages ds
        LEFT JOIN sales_orders so
               ON so.deal_stage_id = ds.id
...
-- ============================================================================
-- 3.  get_sales_dashboard_summary
-- ============================================================================
-- Single-call server-side summary for the Sales Dashboard.  Avoids loading
-- all sales orders and invoices client-side just to compute KPI tiles.
--
-- Return shape (jsonb):
--   {
--     mtd: {
--       order_count, delivered_count, total_value, cancelled_count
--     },
--     vehicles_linked: <count of non-null vehicle_id>,
--     branch_breakdown: [ { branch_code, order_count } ],
--     monthly_trend:    [ { month_key, order_count } ],  -- last 6 months
--     outstanding_ar:   <sum of unpaid/partial invoice outstanding amounts>
--   }
-- ============================================================================
...
COMMENT ON FUNCTION get_sales_dashboard_summary IS
  'Single-call server-side KPI summary for the Sales Dashboard: MTD counts '
  'and revenue, vehicles linked, branch breakdown, 6-month trend, and '
  'outstanding AR. Replaces full client-side sales_orders + invoices '
  'hydration for KPI tiles.';
