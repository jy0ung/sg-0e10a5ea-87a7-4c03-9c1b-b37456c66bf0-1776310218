-- Phase 2 step #18: transactional import commit.
-- One SECURITY DEFINER function performs the vehicles upsert, quality_issues insert,
-- and import_batches status update inside a single transaction. A failure anywhere
-- rolls back cleanly, preventing orphaned batches or half-applied vehicles.
...
begin
  -- Load batch + verify caller is in the same company.
  ...
  -- Upsert vehicles. The jsonb payload is expected to match the vehicles row shape
  -- already produced by importService.validateAndInsertVehicles (snake_case columns).
  ...
  -- Insert quality issues (append-only; no upsert semantics).
  ...
  -- Finalize batch atomically with the inserts above.
...
comment on function public.commit_import_batch(uuid, jsonb, jsonb, integer, integer) is
  'Transactional commit for an import batch: upserts vehicles, inserts quality issues, finalizes batch status. All-or-nothing.';
