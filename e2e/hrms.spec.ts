...
const CURRENT_PAYROLL_DATE = new Date();
const CURRENT_PAYROLL_YEAR = CURRENT_PAYROLL_DATE.getFullYear();
const CURRENT_PAYROLL_MONTH = CURRENT_PAYROLL_DATE.getMonth() + 1;
const CURRENT_PAYROLL_MONTH_LABEL = CURRENT_PAYROLL_DATE.toLocaleString("en-US", { month: "short" });
...
test.describe("Module Directory – HRMS card", () => {
  ...
  test("old HRMS deep links hand off to the dedicated workspace", async ({ page }) => {
    ...
    // HRMS section header link should be visible for super_admin
    await expect(
...
const MOCK_LEAVE_REQUESTS = [
  {
    ...
    employee: { name: "Ahmad Ibrahim" }, leave_types: { name: "Annual Leave" },
  },
  {
    ...
    employee: { name: "Siti Rahimah" }, leave_types: { name: "Sick Leave" },
  },
];
...
test.describe("HRMS – Leave Management", () => {
  ...
  test("pending requests are shown with Approve/Reject actions for managers", async ({ page }) => {
    ...
    const pendingRequestCard = page.locator("[class*='card']").filter({ hasText: /Siti Rahimah/i }).first();

    await expect(pendingRequestCard).toBeVisible({ timeout: 8000 });
    await expect(pendingRequestCard.getByRole("button", { name: /approve/i })).toBeVisible({ timeout: 3000 });
    await expect(pendingRequestCard.getByRole("button", { name: /reject/i })).toBeVisible({ timeout: 3000 });
  });
});
...
const MOCK_ATTENDANCE = [
  {
    ...
    date: TODAY, status: "present", clock_in: "09:00", clock_out: "18:00",
    hours_worked: 9, notes: null,
    created_at: `${TODAY}T09:00:00Z`, updated_at: `${TODAY}T18:00:00Z`,
    profiles: { name: "Ahmad Ibrahim" },
  },
  {
    id: "att-002", company_id: MOCK_PROFILE.company_id, employee_id: "emp-002",
    date: YESTERDAY, status: "absent", clock_in: null, clock_out: null,
    hours_worked: null, notes: "Medical",
    created_at: `${YESTERDAY}T00:00:00Z`, updated_at: `${YESTERDAY}T00:00:00Z`,
...
test.describe("HRMS – Attendance Log", () => {
  ...
  test("Log Attendance button opens form for managers", async ({ page }) => {
    ...
    await page.locator("button").filter({ hasText: /mark.?attendance/i }).first().click();
    await expect(page.locator("[role='dialog']")).toBeVisible({ timeout: 5000 });
  });
});
...
const MOCK_PAYROLL_RUNS = [
  {
    ...
    period_year: CURRENT_PAYROLL_YEAR, period_month: CURRENT_PAYROLL_MONTH, status: "draft",
...
test.describe("HRMS – Payroll Summary", () => {
  ...
  test("shows payroll run from mock data", async ({ page }) => {
    ...
    await expect(page.locator(`text=/${CURRENT_PAYROLL_MONTH_LABEL}/i`).first()).toBeVisible({ timeout: 8000 });
    // Draft badge
...
const MOCK_APPRAISAL_ITEMS = [
  {
    id: "api-001", appraisal_id: "ap-001", employee_id: "emp-001",
    reviewer_id: MOCK_PROFILE.id, rating: 4, status: "pending",
    goals: null, achievements: null, areas_to_improve: null,
    reviewer_comments: null, employee_comments: null, reviewed_at: null,
    profiles: { name: "Ahmad Ibrahim" }, reviewer: { name: "Test Admin" },
  },
];
...
async function setupAppraisalMocks(page: import("@playwright/test").Page) {
  ...
  await page.route(`${SUPABASE_URL}/rest/v1/appraisal_items*`, route => {
    if (route.request().method() === "GET") {
      route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(MOCK_APPRAISAL_ITEMS) });
    } else { route.continue(); }
  });
...
test.describe("HRMS – Performance Appraisals", () => {
  ...
  test("shows appraisal cycle and related review from mock data", async ({ page }) => {
    await page.goto("/hrms/appraisals");
    await page.waitForLoadState("networkidle", { timeout: 15000 }).catch(() => {});

    await expect(page.locator("text=H1 2024")).toBeVisible({ timeout: 8000 });
    await page.locator("button").filter({ hasText: /view reviews/i }).first().click();
    await expect(page.locator("text=Ahmad Ibrahim")).toBeVisible({ timeout: 8000 });
  });
});
...
test.describe("HRMS – Announcements", () => {
  ...
  test("category filter shows only holiday announcements", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /holiday/i }).click();
