...
export const upsertTinType = (cid: string, f: Partial<TinType> & { id?: string }) =>
  f.id ? supabase.from('tin_types').update({ code: f.code, name: f.name, status: f.status, updated_at: new Date().toISOString() } as never).eq('company_id', cid).eq('id', f.id).then(({ error }) => ({ error: error ? new Error(error.message) : null }))
       : supabase.from('tin_types').insert({ id: crypto.randomUUID(), company_id: cid, code: f.code, name: f.name, status: f.status ?? 'Active' } as never).then(({ error }) => ({ error: error ? new Error(error.message) : null }));
...
export const upsertRegistrationFee = async (cid: string, f: Partial<RegistrationFee> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('registration_fees').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('registration_fees').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertRoadTaxFee = async (cid: string, f: Partial<RoadTaxFee> & { id?: string }): Promise<{ error: Error|null }> => {
  const row = { company_id: cid, description: f.description, price: f.price ?? 0, status: f.status ?? 'Active', updated_at: new Date().toISOString() };
  const { error } = f.id ? await supabase.from('road_tax_fees').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('road_tax_fees').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertInspectionFee = async (cid: string, f: Partial<InspectionFee> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('inspection_fees').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('inspection_fees').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertHandlingFee = async (cid: string, f: Partial<HandlingFee> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('handling_fees').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('handling_fees').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertAdditionalItem = async (cid: string, f: Partial<AdditionalItem> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('additional_items').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('additional_items').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertPaymentType = async (cid: string, f: Partial<PaymentType> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('payment_types').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('payment_types').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertBank = async (cid: string, f: Partial<BankRecord> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('banks').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('banks').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertSupplier = async (cid: string, f: Partial<Supplier> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('suppliers').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('suppliers').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertDealer = async (cid: string, f: Partial<Dealer> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('dealers').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('dealers').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertUserGroup = async (cid: string, f: Partial<UserGroup> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('user_groups').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('user_groups').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertDealerInvoice = async (cid: string, f: Partial<DealerInvoice> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('dealer_invoices').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('dealer_invoices').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
...
export const upsertOfficialReceipt = async (cid: string, f: Partial<OfficialReceipt> & { id?: string }): Promise<{ error: Error|null }> => {
  ...
  const { error } = f.id ? await supabase.from('official_receipts').update(row as never).eq('company_id', cid).eq('id', f.id) : await supabase.from('official_receipts').insert({ ...row, id: crypto.randomUUID() } as never);
  return { error: error ? new Error(error.message) : null };
};
