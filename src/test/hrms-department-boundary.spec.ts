...
describe('HRMS Department service boundary', () => {
  it('keeps Department table access out of app-local admin services', () => {
    for (const relativePath of adapters) {
      const source = readFileSync(resolve(process.cwd(), relativePath), 'utf8');
      expect(source).toContain("from '@flc/hrms-services'");
      expect(source).not.toMatch(/\.from\(\s*['"]departments['"]\s*\)/);
    }
  });
});
