...
- Shared status tone classes and low-risk page primitives (`PageHeader`, `PageSpinner`, `KpiSkeleton`, `TableSkeleton`, `ScrollableRegion`, `LocationPreservingNavigate`, `UnauthorizedAccess`, `StepperProgress`, `AuditDiffTable`, `MobileCardList`, `KpiCard`, `HrmsEmptyState`) are owned by `@flc/ui`; app-local files are compatibility re-exports guarded by `check:ui-boundary` for incremental UI migration.
