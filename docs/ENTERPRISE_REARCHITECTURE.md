...
- Shared status tone classes and low-risk page primitives (`PageHeader`, `PageSpinner`) are owned by `@flc/ui`; app-local files are compatibility re-exports guarded by `check:ui-boundary` for incremental UI migration.
